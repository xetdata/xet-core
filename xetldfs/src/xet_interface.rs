...
use crate::path_utils::{path_of_fd, resolve_path, verify_path_is_git_repo};
use crate::runtime::{tokio_run, with_interposing_disabled};
use crate::xet_rfile::XetFdReadHandle;
use crate::{char_buf_to_str, cstring_to_str, real_close};
...
lazy_static! {
    static ref XET_REPOS: Vec<([c_char; PATH_BUF_SIZE], RwLock<Option<XetFSRepoWrapper>>)> =
...
fn initialize_repo_paths() -> Vec<(
    ...
    for path in paths {
        ...
        let Ok(c_path) = CString::new(path) else {
            ld_error!("Malformed path in XET_LDFS_REPO ({path}); skipping");
            continue;
        };
        ...
        if result.is_null() {
            ld_error!("Error accessing path {path} in XET_LDFS_REPO; skipping.");
            continue;
        }

        if !verify_path_is_git_repo(char_buf_to_str(&resolved_path)) {
            ld_error!("Path {path} does not appear to be a git repo, skipping.");
            continue;
        }

        // Push the tuple into the vector
        repos.push((resolved_path, RwLock::new(None)));
    }
