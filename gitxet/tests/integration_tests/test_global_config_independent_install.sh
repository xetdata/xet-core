...
# First, we run a global git install 
git xet install 


# Now, let's deal with the repos
...
git xet init --force # This should write to the local config file, but the global one should be modified above.
