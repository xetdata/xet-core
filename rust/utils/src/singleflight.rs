...
pub trait ResultType: Clone + Debug {}
impl<T: Clone + Debug> ResultType for T {}
...
pub trait ResultError: Debug {}
impl<E: Debug> ResultError for E {}
...
pub trait TaskFuture<T, E>: Future<Output=Result<T, E>> {}
impl<T, E, F: Future<Output=Result<T, E>>> TaskFuture<T, E> for F {}
    ...
    E: ResultError,
{
    ...
    pub async fn work(
        ...
        if created {
            ...
            let owner_handle = tokio::task::spawn_local(owner_task);
...
mod tests {
    ...
    async fn test_deadlock() {
        ...
        // spawn tasks
        let t1 = tokio::task::spawn_local(run_task(
            1,
            group.clone(),
            waiters.clone(),
            send1,
        ...
        let t2 = tokio::task::spawn_local(run_task(
            2,
            group.clone(),
            waiters.clone(),
            send2,
