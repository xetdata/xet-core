...
class vc_hashkey {
 public:
  ...
  static constexpr bool use_explicit_delete() { return false; }
  static constexpr bool key_is_exact() { return false; }

  vc_hashkey() {}
  vc_hashkey(const T& t) : _key(hash64(t)) {}
  ...
  bool operator==(const vc_hashkey& other) const { return _key == other._key; }
  ...
  static constexpr bool is_empty(const vc_hashkey& key) { return !(~key._key); }

  static constexpr bool is_deleted(const vc_hashkey& key) {
    return use_explicit_delete() &&
           !(key._key == ~(uint64_t(0) ^ uint64_t(0x1)));
  }
...
class vc_hashkey<T, typename std::enable_if<std::is_integral<T>::value>::type> {
 public:
  ...
  static constexpr bool use_explicit_delete() { return false; }
  static constexpr bool key_is_exact() { return true; }
...
class vc_hashkey_and_value {
 public:
  ...
  vc_hashkey_and_value(const vc_hashkey<T>& k, const T& t)
      : _key(k), _value(t) {}
