...
#include "value_container_mapper_internal.hpp"
...
class value_container_mapper {
 public:
   ...
   */
  void insert(const hashkey_and_value& hv, ValueContainer* v_ptr) {
   ...
   *   \overload
   */
  void insert(const hashkey& hk, ValueContainer* v_ptr) { _insert(hk, v_ptr); }

  /** Returns the container associated with this key and value.  If
   *  it's not present or has been invalidated, returns nullptr.
   */
  ValueContainer* find(const hashkey_and_value& hv) {
    return _find_reference<ValueContainer*, value_container_mapper>(
   ...
   *  \overload
   */
  ValueContainer* find(const hashkey& key, const Value& t) {
    return _find_reference<ValueContainer*, value_container_mapper>(this, key,
                                                                    t);
  }
   ...
   *  \overload
   */
  const ValueContainer* find(const hashkey_and_value& hv) const {
    return _find_reference<const ValueContainer*, const value_container_mapper>(
        this, hv.key(), hv.value());
  }
   ...
   *  \overload
   */
  const ValueContainer* find(const hashkey& key, const Value& t) const {
    return _find_reference<const ValueContainer*, const value_container_mapper>(
        this, key, t);
  }
   ...
   */
  void invalidate(const hashkey_and_value& hv, ValueContainer* v_ptr) {
    DASSERT_TRUE(hv.value() == v_ptr->get_hashkey_and_value().value());
    DASSERT_TRUE(hv.key() == v_ptr->get_hashkey_and_value().key());
   ...
   *   \overload
   */
  void invalidate(const hashkey& hk, ValueContainer* v_ptr) {
 ...
 private:
  ...
  struct gdhs_hash {
    size_t operator()(const internal_value_type& kv_pair) const {
      return kv_pair.first.hash();
    }
  };

  struct gdhs_equality {
    bool operator()(const internal_value_type& kv_pair_1,
                    const internal_value_type& kv_pair_2) const {
      if (hashkey::key_is_exact()) {
        return kv_pair_1.first == kv_pair_2.first;

      } else {
        if (kv_pair_1.first != kv_pair_2.first) return false;

        if (hashkey::use_explicit_delete()) {
          if (kv_pair_1.second == nullptr && kv_pair_2.second == nullptr)
            return true;

          return (kv_pair_1.second != nullptr && kv_pair_2.second != nullptr &&
          ...
          return (kv_pair_1.second != nullptr && kv_pair_2.second != nullptr &&
                  kv_pair_1.second->get_hashkey_and_value().key() ==
                      kv_pair_1.first &&
                  kv_pair_2.second->get_hashkey_and_value().key() ==
                      kv_pair_2.first &&
                  (kv_pair_1.second->get_hashkey_and_value().value() ==
                   kv_pair_2.second->get_hashkey_and_value().value()));
        }
  ...
  typedef google::dense_hash_set<internal_value_type, gdhs_hash, gdhs_equality>
      hash_map_type;
 ...
 public:
  value_container_mapper()
    ...
    if (hashkey::use_explicit_delete()) table.set_deleted_key(_kv_deleted);
 ...
 private:
  ...
  /**  Marks a key, value_container as invalid. It is assumed, however, that as
   * long as ValueContainer* holds the value, it is a valid key; otherwise it is
   * not.  This function does some lazy cleanup, but may not erase the key.
   */
  void _invalidate(const hashkey& key, ValueContainer* v_ptr) {
    internal_value_type kv_pair{key, v_ptr};

    if ((hashkey::is_empty(kv_pair.first))) {
      _erase_in_stack(kv_pair, _kv_empty, _kv_empty_overflow_stack);
      return;
    }

    if ((hashkey::is_deleted(kv_pair.first))) {
    ...
    if (hashkey::use_explicit_delete()) {
      ...
      if ((erase_counter >= reserved_size)) {
        refresh_hash_table();
        erase_counter = 0;
      }
    }
   ...
   */
  void _insert(const hashkey& key, ValueContainer* v_ptr) {
    internal_value_type kv_pair{key, v_ptr};
  ...
  inline size_t _size() const {
    return ((_kv_empty.second == nullptr ? 0 : 1) +
            ...
            _kv_deleted_overflow_stack.size() + table.size());
  }
 ...
 private:
  ...
  void _erase_in_stack(const internal_value_type& kv_pair,
                       ...
                       std::vector<internal_value_type>& stack) {
    if (hashkey::key_is_exact()) {
  ...
  static _ReferenceType _find_reference(_ThisType* c, const hashkey& key,
                                        const Value& t) {
    if (hashkey::key_is_exact()) {
      ...
      return ((it == c->table.end() ||
               (!hashkey::use_explicit_delete() &&
                it->first != it->second->get_hashkey_and_value().key()))
      ...
      typedef typename std::conditional<std::is_const<_ThisType>::value,
                                        ...
                                        internal_value_type&>::type iv_ref;

      typedef typename std::conditional<std::is_const<_ThisType>::value,
                                        const std::vector<internal_value_type>&,
                                        std::vector<internal_value_type>&>::type
          iv_vect_ref;

      // Set in one of the local stacks.
      auto find_in_stack = [&](iv_ref kv_base, iv_vect_ref stack) {
  ...
  // Set in one of the local stacks.
  void _insert_in_stack(const internal_value_type& v,
                        internal_value_type& kv_base,
                        std::vector<internal_value_type>& stack) {
    if (hashkey::key_is_exact()) {
      DASSERT_TRUE(kv_base.second == nullptr);
