...
use crate::diff::error::DiffError::{FailedSummaryCalculation, NoSummaries};
...
impl SummaryFetcher {
    pub async fn new(repo: Arc<GitXetRepo>) -> Result<Self, DiffError> {
        Ok(Self {
            db: Self::load_db(&repo).await?,
            repo,
        })
    }

    async fn load_db(repo: &Arc<GitXetRepo>) -> Result<WholeRepoSummary, DiffError> {
