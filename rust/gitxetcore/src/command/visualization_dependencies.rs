...
pub async fn visualization_dependencies_command(
    ...
    if let (false, Ok(possible_note)) = (args.no_cache_read, repo.find_note(Some(notes_ref), oid)) {
        ...
        tree.walk(git2::TreeWalkMode::PreOrder, |root, entry| {
            if let Some(git2::ObjectType::Blob) = entry.kind() {
                if let Ok(blob) = entry.to_object(&repo).and_then(|x| x.peel_to_blob()) {
        ...
        if !args.no_cache_write {
            ...
            repo.note(&sig, &sig, Some(notes_ref), oid, &deps_str, false)?;
        }
        println!("{deps_str}");
    }
    Ok(())
