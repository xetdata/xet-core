...
fn impl_struct(input: Struct) -> TokenStream {
    ...
    let source_method = source_body.map(|body| {
        quote! {
            fn source(&self) -> ::core::option::Option<&(dyn std::error::Error + 'static)> {
                use xet_error::__private::AsDynError;
                #body
            }
        }
    });
    ...
    let provide_method = input.backtrace_field().map(|backtrace_field| {
        ...
        let body = if let Some(source_field) = input.source_field() {
            ...
            let source_provide = if type_is_option(source_field.ty) {
                quote_spanned! {source.member_span()=>
                    if let ::core::option::Option::Some(source) = &self.#source {
                        source.xet_error_provide(#request);
                    }
                }
            } else {
                quote_spanned! {source.member_span()=>
                    self.#source.xet_error_provide(#request);
                }
            };
            ...
            quote! {
                use xet_error::__private::ThiserrorProvide;
                #source_provide
                #self_provide
            }
        } else if type_is_option(backtrace_field.ty) {
        ...
        quote! {
            ...
            impl #impl_generics ::core::convert::From<#from> for #ty #ty_generics #where_clause {
                ...
                fn from(source: #from) -> Self {
                    xet_error::error_hook(&format!("{source:?}"));
                    #ty #body
                }
            }
        }
            ...
            match (variant.backtrace_field(), variant.source_field()) {
                    ...
                    if backtrace_field.attrs.backtrace.is_none() =>
                {
                    ...
                    let source_provide = if type_is_option(source_field.ty) {
                        ...
                        quote_spanned! {source.member_span()=>
                            #varsource.xet_error_provide(#request);
                        }
                    };
...
fn use_as_display(needs_as_display: bool) -> Option<TokenStream> {
    if needs_as_display {
        Some(quote! {
            use xet_error::__private::AsDisplay as _;
        })
    } else {
        None
    }
